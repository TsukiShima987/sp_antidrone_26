...
BaseDetector::BaseDetector() {
    ...
    // 优先使用标定覆盖文件，若不存在则使用原始配置
    std::ifstream calib_file(calib_path_);
    if (calib_file.good()) {
        calib_file.close();
        const auto calib = YAML::LoadFile(calib_path_);
        auto laser_offset_data = calib["laser_offset"].as<std::vector<double>>();
        auto laser_direction_data = calib["laser_direction"].as<std::vector<double>>();
        S0 = Eigen::Vector3d(laser_offset_data.data());
        d0 = Eigen::Vector3d(laser_direction_data.data());
        std::cout << "[LaserCalib] Using calibrated values from "
                  << calib_path_ << std::endl;
    } else {
        ...
        S0 = Eigen::Vector3d(laser_offset_data.data());
        d0 = Eigen::Vector3d(laser_direction_data.data());
        std::cout << "[LaserCalib] Using original values from antidrone.yaml" << std::endl;
    }
    d0.normalize();
}
...
void BaseDetector::adjustLaserDirection(double d_pitch, double d_yaw) {
    // d_pitch: 绕相机 X 轴旋转 (上下调整)
    // d_yaw:   绕相机 Y 轴旋转 (左右调整)
    Eigen::AngleAxisd rot_x(d_pitch, Eigen::Vector3d::UnitX());
    Eigen::AngleAxisd rot_y(d_yaw,   Eigen::Vector3d::UnitY());
    d0 = rot_y * rot_x * d0;
    d0.normalize();
}

bool BaseDetector::saveCalibration(const std::string& calib_path) {
    YAML::Emitter out;
    out << YAML::BeginMap;
    out << YAML::Key << "laser_offset" << YAML::Value
        << YAML::Flow << std::vector<double>{S0.x(), S0.y(), S0.z()};
    out << YAML::Key << "laser_direction" << YAML::Value
        << YAML::Flow << std::vector<double>{d0.x(), d0.y(), d0.z()};
    out << YAML::EndMap;

    std::ofstream fout(calib_path);
    if (!fout) {
        std::cerr << "[LaserCalib] Failed to open " << calib_path << " for writing" << std::endl;
        return false;
    }
    fout << "# Auto-generated calibration override\n"
         << "# Generated by keyboard calibration. Delete this file to reset.\n";
    fout << out.c_str();
    fout.close();
    std::cout << "[LaserCalib] Saved to " << calib_path << std::endl;
    return true;
}

void BaseDetector::reloadLaserParams(const std::string& config_path) {
    const auto config = YAML::LoadFile(config_path);
    auto laser_offset_data = config["laser_offset"].as<std::vector<double>>();
    auto laser_direction_data = config["laser_direction"].as<std::vector<double>>();
    S0 = Eigen::Vector3d(laser_offset_data.data());
    d0 = Eigen::Vector3d(laser_direction_data.data());
    d0.normalize();
    std::cout << "[LaserCalib] Reloaded laser params from " << config_path << std::endl;
}
