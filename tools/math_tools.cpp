...
Eigen::Vector3d ypd2xyz(const Eigen::Vector3d & ypd)
{
  ...
  auto z = distance * std::cos(pitch) * std::cos(yaw);
  auto x = distance * std::cos(pitch) * std::sin(yaw);
  auto y = distance * std::sin(pitch);
  return {x, y, z};
}
...
cv::Point2f rotate_point_clockwise(
  const cv::Point2f & point, const cv::Point2f & rotation_center, double angle_radians)
{
  double x_rel = point.x - rotation_center.x;
  double y_rel = point.y - rotation_center.y;

  double cos_angle = std::cos(angle_radians);
  double sin_angle = std::sin(angle_radians);

  double x_rotated = (x_rel * cos_angle) - (y_rel * sin_angle);
  double y_rotated = (x_rel * sin_angle) + (y_rel * cos_angle);

  return cv::Point2f(
    static_cast<float>(x_rotated + rotation_center.x),
    static_cast<float>(y_rotated + rotation_center.y));
}
}  // namespace tools